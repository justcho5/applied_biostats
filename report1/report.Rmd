...
```{r, echo=FALSE, fig.cap="Interaction plot"}
op <-par(mfrow = c(3,1))
?par()
par(mar=c(2,2,2,5))
...
Furthermore, although the moisture content is lower for slow transpiration for all types of cutes, the difference in moisture content between rapid and slow transpiration is greater when the location of the cut is central.
...
```
We find that the combined effect of the three-way interactions is not significant (p > 0.05), so we continue on with the model with the main effects only.
